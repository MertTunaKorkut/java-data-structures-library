...
public class AVLDictionary<K, V> extends AbstractSortedDictionary<K, V> implements SortedDictionary<K, V> {
  ...
  public static <K, V> AVLDictionary<K, V> copyOf(AVLDictionary<K, V> that) {
    AVLDictionary<K,V> avlDic = empty(that.comparator);

    Iterable<Entry<K,V>> preOrder = that.avlTree.preOrder();

    for(Entry<K,V> entry : preOrder){
      avlDic.insert(entry);
    }
   ...
   */
  public static <K extends Comparable<? super K>, V> AVLDictionary<K, V> copyOf(SortedDictionary<K, V> that) {

    AVLDictionary<K,V> avlDic = empty();
  ...
  public V valueOf(K key) {
    Entry<K,V> tempEnt = Entry.withKey(key);

    Entry<K,V> searchResult = avlTree.search(tempEnt);
  ...
  public boolean isDefinedAt(K key) {
    ...
    return avlTree.contains(tempEnt);
  }
  ...
  public Entry<K, V> minimum() {
    if(isEmpty()){
      throw new NoSuchElementException("minimum on empty avldictionary");
    }
