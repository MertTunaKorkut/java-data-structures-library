...
public class WBLeftistHeap<T> implements Heap<T> {
  ...
  private static <T> WBLeftistHeap<T> merge(Comparator<T> comparator, JDKArrayList<Node<T>> nodes) {
      ...
      WBLeftistHeap<T> heap = WBLeftistHeap.empty(comparator);

      if(nodes.isEmpty()){
          return heap;
      }

      JDKQueue<Node<T>> q = JDKQueue.from(nodes);
  ...
  private Node<T> merge(Node<T> node1, Node<T> node2) {
      ...
      if(weight(node1.left) < weight(node1.right)){
          Node<T> temp = node1.left;
          node1.left = node1.right;
          node1.right = temp;
      }

      node1.weight = weight(node1.left) + weight(node1.right) + 1;
