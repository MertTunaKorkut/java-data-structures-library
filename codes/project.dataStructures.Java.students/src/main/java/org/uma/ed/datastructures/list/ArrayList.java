...
public class ArrayList<T> extends AbstractList<T> implements List<T> {
  ...
  public static <T> ArrayList<T> copyOf(List<T> that) {
      ...
      ArrayList<T> arrayList = withCapacity(that.size());

      /*
      for (int i = 0; i < that.size(); i++) {
          arrayList.append(that.get(i));
      }
      */
  ...
  public void delete(int index) {
      ...
      if(isEmpty()){
          throw new ListException("delete on empty list");
      }
  ...
  public T get(int index) {
      if(isEmpty()){
          throw new ListException("get on empty list");
      }
      validateIndex(index);
