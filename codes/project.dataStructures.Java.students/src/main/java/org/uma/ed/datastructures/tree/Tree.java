...
public final class Tree {
  ...
  public static <T> int height(Node<T> root) {
      ...
      for(Node<T> child: root.children){
          ...
          childMaxHeight = Math.max(childMaxHeight, height(child));
      }
  ...
  public static <T> T maximum(Node<T> root, Comparator<T> comparator) {
      ...
      for(Node<T> child: root.children){
          ...
          T childMax = maximum(child,comparator);

          maxSoFar = comparator.compare(maxSoFar, childMax ) > 0 ? maxSoFar : childMax;
      }
  ...
  public static <T> List<T> breadthFirst(Node<T> root) {
      ...
      while(!queue.isEmpty()){
          ...
          traversal.append(current.element);
