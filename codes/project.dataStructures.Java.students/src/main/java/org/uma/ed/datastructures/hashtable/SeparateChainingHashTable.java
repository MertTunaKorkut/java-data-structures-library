...
public class SeparateChainingHashTable<K> implements HashTable<K> {
  ...
  public static <K> SeparateChainingHashTable<K> copyOf(SeparateChainingHashTable<K> that) {
    SeparateChainingHashTable<K> hashtable = new SeparateChainingHashTable<>(that.table.length, that.maxLoadFactor);

    if (that.isEmpty()){
      return hashtable;
    }

    for(int i = 0; i < that.table.length; i++){

      if(that.table[i] != null) {
        hashtable.table[i] = new Node<>(that.table[i].key, null);

        Node<K> prev = hashtable.table[i];
        Node<K> current = that.table[i].next;

        while (current != null) {
          Node<K> newNode = new Node<>(current.key, null);
          prev.next = newNode;
  ...
  public static <K> SeparateChainingHashTable<K> copyOf(HashTable<K> that) {
    ...
    if(that instanceof SeparateChainingHashTable<K>){
      return SeparateChainingHashTable.copyOf(that);
    }

    SeparateChainingHashTable<K> hashTable = withCapacity(!that.isEmpty() ? that.size() : DEFAULT_NUM_CHAINS);
  ...
  public void insert(K key) {
    ...
    if(f.current != null){ //already exists
      f.current.key = key;
    }
    else{ //doesn't exist

      Node<K> node = new Node<>(key,null);

      if(f.previous == null){ //that index is empty
        table[f.index] = node;
      }
  ...
  public K search(K key) {
    ...
    return f.current == null ? null : f.current.key;
  }
