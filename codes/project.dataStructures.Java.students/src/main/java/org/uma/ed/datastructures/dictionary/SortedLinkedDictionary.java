...
public class SortedLinkedDictionary<K, V> extends AbstractSortedDictionary<K, V> implements SortedDictionary<K, V> {
  ...
  public static <K, V> SortedLinkedDictionary<K, V> copyOf(SortedDictionary<K, V> that) {
      ...
      SortedLinkedDictionary<K,V> dic = empty(that.comparator());

      if(that.isEmpty()){
          return dic;
      }

      // Node<K,V> prev = dic.first;

      for(Entry<K,V> ent : that){

          dic.append(ent);

          /*
          Node<K,V> node = new Node<>(ent,null);
  ...
  public void insert(Entry<K, V> entry) {
      ...
      if(f.found){
          f.current.entry = entry;
      }
      else{

          Node<K,V> node = new Node<>(entry,f.current);

          if(isEmpty()){ //boşa koy
              first = node;
              last = node;
          }
          else if(f.previous == null){//başa koy
              first = node;
          }
          else if (f.current == null) {//sona koy
              last.next = node;
              last = node;
          }
          else{//ortaya koy
  ...
  public V valueOf(K key) {
      ...
      if(isEmpty()){
          throw new UnsupportedOperationException("valueOf on empty dictionary");
      }
  ...
  public boolean isDefinedAt(K key) {
      ...
      if(isEmpty()){
          throw new UnsupportedOperationException("isDefinedAt on empty dictionary");
      }
  ...
  public void delete(K key) {
      if(isEmpty()){
          throw new UnsupportedOperationException("delete on empty dictionary");
      }

      Finder f = new Finder(key);

      if(f.found){
          if(f.previous == null){ //baştan silme
              first = first.next;
              if(first == null){ //tek eleman vardıysa
                  last = null;
              }
          }
          else if(f.current == last){ //sondan silme
