...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
      BinaryHeap<T> heap = empty(that.comparator());

      heap.elements = Arrays.copyOf(that.elements, that.size());
      heap.size = that.size();
  ...
  private void heapifyDown(int index) {
      ...
      while(hasLeftChild(cur)){
          ...
          int smallerChildIndex = leftChild(cur);
          int rightChildIndex = rightChild(cur);

          if(rightChildIndex < size && lessThan(rightChildIndex,smallerChildIndex)){
              smallerChildIndex = rightChildIndex;
          }

          if(lessThan(cur,smallerChildIndex)){
              break;
          }

          swap(cur,smallerChildIndex);
