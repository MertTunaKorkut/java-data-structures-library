...
public class LinearProbingHashTable<K> implements HashTable<K> {
  ...
  public static <K> LinearProbingHashTable<K> copyOf(LinearProbingHashTable<K> that) {
    LinearProbingHashTable<K> hashTable = new LinearProbingHashTable<>(that.keys.length, that.maxLoadFactor);

    if(!that.isEmpty()){
      System.arraycopy(that.keys,0,hashTable.keys,0, that.keys.length);
      hashTable.size = that.size();
    }
  ...
  public static <K> LinearProbingHashTable<K> copyOf(HashTable<K> that) {
    if(that instanceof LinearProbingHashTable<K>){
      return LinearProbingHashTable.copyOf((LinearProbingHashTable<K>) that);
    }

    LinearProbingHashTable<K> hashTable = withCapacity(!that.isEmpty() ? that.size() : DEFAULT_NUM_CELLS);
  ...
  public boolean contains(K key) {
    ...
    return keys[searchIndex(key)] != null;
  }
  ...
  public void delete(K key) {
    ...
    if(keys[index] != null){ //the key exists
      keys[index] = null; //delete the key
      size--; //decrement the size

      int reInsertIndex = advance(index); //index for re-insertions

      while(keys[reInsertIndex] != null){ //continue until the cluster ends

        K tempKey = keys[reInsertIndex]; //save the key temporarily

        keys[reInsertIndex] = null; //delete the key from the array

        insert(tempKey); //re-insert the key
        size--; //decrement the size each time because insert increments

        reInsertIndex = advance(reInsertIndex); //advance the re-insertion index
      }
