...
public class ArrayQueue<T> extends AbstractQueue<T> implements Queue<T> {
  ...
  public static <T> ArrayQueue<T> copyOf(ArrayQueue<T> that) {
      ...
      if(that.isEmpty()){
          ...
          ArrayQueue<T> queue = of(that.elements);
          queue.first = that.first;
          queue.last = that.last;
          queue.size = that.size;
  ...
  public static <T> ArrayQueue<T> copyOf(Queue<T> that) {
      ArrayQueue<T> queue = new ArrayQueue<>(!that.isEmpty() ? that.size() : DEFAULT_INITIAL_CAPACITY);
      ArrayQueue<T> temp = new ArrayQueue<>(!that.isEmpty() ? that.size() : DEFAULT_INITIAL_CAPACITY);

      while(!that.isEmpty()){
          temp.enqueue(that.first());
          that.dequeue();
      }

      while(!temp.isEmpty()){
          queue.enqueue(temp.first());
          that.enqueue(temp.first());
  ...
  public void clear() {
      if(isEmpty()){
          throw new EmptyQueueException("clear on empty queue");
      }
