...
public class BST<K> implements SearchTree<K> {
  ...
  public static <K> BST<K> copyOf(SearchTree<K> that) {
      BST<K> bst = empty(that.comparator());

      if(that.isEmpty()){
          return bst;
      }
      Iterable<K> iterable = that.preOrder();
  ...
  /* using a helper function is unnecessary
  ...
  public K minimum() {
      if(isEmpty()){
          throw new EmptySearchTreeException("no minimum element");
      }
  ...
  public K maximum() {
      if(isEmpty()){
          throw new EmptySearchTreeException("no maximum element");
      }
