...
public class SortedLinkedSet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public static <T> SortedLinkedSet<T> copyOf(SortedSet<T> that) {
      ...
      SortedLinkedSetBuilder<T> set = new SortedLinkedSetBuilder<>(that.comparator());

      for(T e: that){
          set.append(e);
      }

      return set.toSortedLinkedSet();
  }
  ...
  public void insert(T element) {
      ...
      if(f.found){
          f.current.element = element;
      }
      else{
          Node<T> node = new Node<>(element, f.current);
  ...
  public static <T> SortedLinkedSet<T> union(SortedSet<T> set1, SortedSet<T> set2) {
    ...
    SortedLinkedSetBuilder<T> set = new SortedLinkedSetBuilder<>(set1.comparator());
